...
define(function (require, exports, module) {
  ...
  EditorWrapper.prototype.change = function (filePath, change, content) {
    ...
    if (filePath === self.currentFilePath && self.currentEditor) {
      self._mutex = true
      self.currentEditor.document.replaceRange(change.text, change.from, change.to)
  ...
  EditorWrapper.prototype.createFile = function (filePath, content) {
    ...
    if (filePath === self.currentFilePath && self.currentEditor) {
      self._mutex = true
      self.currentEditor.document.refreshText(content, self.currentEditor.document.diskTimestamp, true)
      self._mutex = false
    } else {
  ...
  EditorWrapper.prototype._onActiveEditorChange = function ($event, newEditor, oldEditor) {
    ...
    if (newEditor) {
      ...
      if (self.currentFilePath) {
        ...
        self.emit('changeFile', self.currentFilePath, emptyDelta)
      } else if(self.currentEditor.document.isUntitled) {
        self.currentEditor._codeMirror.on('change', self._onUnsavedChange.bind(self))
        self.emit('alert', 'alert-unsaved')
      }
    }
  }
  
  EditorWrapper.prototype._onUnsavedChange = function (cm, change) {
    var self = this
    
    self.emit('alert', 'alert-unsaved') // remind them again
  }
