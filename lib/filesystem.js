...
define(function (require, exports, module) {
  ...
  var SaveCommand = brackets.getModule("command/CommandManager").get(FILE_SAVE)
  ...
  FileSystemWrapper.prototype.getProject = function (cb) {
    ProjectManager.getAllFiles().then(function (allFiles) {
      ...
      allFiles = allFiles.filter(function (a) {
        for (var i=0; i<FILE_BLACKLIST.length; i++) {
          if (a.fullPath.indexOf(FILE_BLACKLIST[i]) !== -1) {
            return false
          }
        }
        return true
      ...
      for (var i = 0; i < allFiles.length; i++) {
        ;(function (i) {
          allFiles[i].read(function (err, contents, stat) {
            ...
            cb(filePath, contents, {current: i+1, total: allFiles.length})
    ...
    DocumentManager.getDocumentForPath(absPath).then(function (doc) {
      FileSystem.resolve(absPath, function (err, entry) {
        ...
        if (entry && entry.moveToTrash) {
          self.saveDocument(doc, function () { // prevent unsaved changes
            entry.moveToTrash()
  ...
  FileSystemWrapper.prototype._pushChangeToDocument = function (absPath, change, saveAfter) {
    ...
    return DocumentManager.getDocumentForPath(absPath).then(function (doc) {
      ...
      if (change.fn === 'setValue') {
        ...
        if (document.querySelector('#toolbar-go-live').title !== 'Live Preview') {
      ...
      // ignore useless save message
      ;[].forEach.call(document.querySelectorAll('.modal-wrapper'), function(e){
        e.querySelector('.dialog-button.btn.left').click()
      })
    })
  }
  
  FileSystemWrapper.prototype.saveDocument = function (doc, cb) {
    if(!doc.isDirty) return cb()
